...
- Breaking beta.1 migration: regenerate every checked-in `*.typed-sql.ts` file. Scalar profiles now describe the driver contract explicitly, and runtime execution consumes PostgreSQL's positional result rows so duplicate and otherwise non-object-safe column names remain exact.
