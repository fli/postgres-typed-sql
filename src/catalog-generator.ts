...
function catalogResultTypeResolution(
  ...
  if (scalarProfile === 'node-postgres' && type.pgTypeKind === 'domain' && type.pgBaseType) {
    return catalogResultTypeResolution(type.pgBaseType, scalarProfile, renderedEnumOids)
  }
  if (scalarProfile === 'node-postgres' && type.pgTypeKind === 'enum' && renderedEnumOids.has(type.pgTypeOid)) {
    return {
      ambientBindings: [],
      scalarImports: [],
      type: postgresNamedTypeBinding(type.pgTypeSchema, type.pgTypeName),
    }
  }
  return resolveTypeScriptResultTypeForPostgresType(type, scalarProfile)
}
...
function tsTypeForColumn(
  ...
  const resolution = catalogResultTypeResolution(type, scalarProfile, renderedEnumOids)
...
function renderCatalogTypes(
  ...
  const resolvedColumns = columns.map((row): ResolvedColumnCatalogRow => {
    ...
    const resolution = tsTypeForColumn(row, type, checkConstraintTypes, scalarProfile, renderedEnumOids)
